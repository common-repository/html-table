(function($, pointers) {
	...
	pointers.setPointer = function() {
		...
		$pointer = $target.pointer({
			...
			show: function(event, t) {
				...
				if(trigger) {
					$('.vwklhtmltables-plugin .tabs-wrapper li:eq('+ trigger +') .button').trigger('click');
				}
			},
			close: function(event) {
				if (pointers.hasNextStep) {
					pointers.stepNumber += 1;
					sessionStorage.setItem('vwklhtmltables-tables-tutorial-step', pointers.stepNumber);
					...
					$.post(ajaxurl, {
						action: 'vwklhtmltables-tables-tutorial-close'
					});
					pointers.stepNumber = 0;
					sessionStorage.removeItem('vwklhtmltables-tables-tutorial-step');
				}
			}
		});
	...
	stepNumber = sessionStorage.getItem('vwklhtmltables-tables-tutorial-step');
	...
	var fromBegin = getUrlParameter('vwklhtmltables_tutorial');
	...
	pointers.actions = {
		'step-0': function() {
			pointers = this;
			$('#toplevel_page_vwklhtmltables-tables').on('click', 'a', function(event) {
